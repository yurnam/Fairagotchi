...
class Display(View):
    def __init__(self, config, state={}):
        ...
        # --- added: periodic PNG output (for fb0 bridge) ---
        self._png_path = "/var/tmp/pwnagotchi.png"
        self._png_every = 1.0  # seconds between writes (tweak as you like)
        self._last_png = 0
        # --------------------------------------------------
    ...
    def _on_view_rendered(self, img):
        # --- periodic PNG output (for fb0 bridge) ---
        try:
            now = time.time()
            if now - self._last_png >= self._png_every:
                self._last_png = now

                # rotate 90° for phone framebuffer
                out = img.rotate(90, expand=True)

                tmp = self._png_path + ".tmp"
                out.save(tmp, "PNG")
                os.replace(tmp, self._png_path)
        except Exception as e:
            logging.error("png frame save failed: %s" % e)
        # --------------------------------------------------
