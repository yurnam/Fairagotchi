...
static const struct regulator_bulk_data nt35532_cs_supplies[] = {
	{ .supply = "vsn" },
	{ .supply = "vsp" },
};

static inline struct nt35532_cs *to_nt35532_cs(struct drm_panel *panel)
{
	return container_of(panel, struct nt35532_cs, panel);
}

static void nt35532_cs_reset(struct nt35532_cs *ctx)
{
...
static int nt35532_cs_on(struct nt35532_cs *ctx)
{
...
static int nt35532_cs_off(struct nt35532_cs *ctx)
{
	struct mipi_dsi_multi_context dsi_ctx = { .dsi = ctx->dsi };
...
static int nt35532_cs_prepare(struct drm_panel *panel)
{
	struct nt35532_cs *ctx = to_nt35532_cs(panel);
	struct device *dev = &ctx->dsi->dev;
	int ret;

	ret = regulator_bulk_enable(ARRAY_SIZE(nt35532_cs_supplies), ctx->supplies);
	if (ret < 0) {
		dev_err(dev, "Failed to enable regulators: %d\n", ret);
		return ret;
	}

	nt35532_cs_reset(ctx);

	ret = nt35532_cs_on(ctx);
	if (ret < 0) {
		dev_err(dev, "Failed to initialize panel: %d\n", ret);
		gpiod_set_value_cansleep(ctx->reset_gpio, 1);
		regulator_bulk_disable(ARRAY_SIZE(nt35532_cs_supplies), ctx->supplies);
		return ret;
	}
...
static int nt35532_cs_unprepare(struct drm_panel *panel)
{
	struct nt35532_cs *ctx = to_nt35532_cs(panel);
	struct device *dev = &ctx->dsi->dev;
	int ret;

	ret = nt35532_cs_off(ctx);
	if (ret < 0)
...
static const struct drm_display_mode nt35532_cs_mode = {
...
static int nt35532_cs_get_modes(struct drm_panel *panel,
				struct drm_connector *connector)
{
	return drm_connector_helper_get_modes_fixed(connector, &nt35532_cs_mode);
}

static const struct drm_panel_funcs nt35532_cs_panel_funcs = {
	.prepare = nt35532_cs_prepare,
	.unprepare = nt35532_cs_unprepare,
	.get_modes = nt35532_cs_get_modes,
};

static int nt35532_cs_probe(struct mipi_dsi_device *dsi)
{
	struct device *dev = &dsi->dev;
	struct nt35532_cs *ctx;
	int ret;

	ctx = devm_drm_panel_alloc(dev, struct nt35532_cs, panel,
				   &nt35532_cs_panel_funcs,
				   DRM_MODE_CONNECTOR_DSI);
	if (IS_ERR(ctx))
		return PTR_ERR(ctx);

	ret = devm_regulator_bulk_get_const(dev,
					    ARRAY_SIZE(nt35532_cs_supplies),
					    nt35532_cs_supplies,
					    &ctx->supplies);
	if (ret < 0)
		return ret;
...
static void nt35532_cs_remove(struct mipi_dsi_device *dsi)
{
	struct nt35532_cs *ctx = mipi_dsi_get_drvdata(dsi);
	int ret;
...
static const struct of_device_id nt35532_cs_of_match[] = {
	{ .compatible = "billion,rimob-nt35532-cs" }, // FIXME
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, nt35532_cs_of_match);

static struct mipi_dsi_driver nt35532_cs_driver = {
	.probe = nt35532_cs_probe,
	.remove = nt35532_cs_remove,
	.driver = {
		.name = "panel-nt35532-cs",
		.of_match_table = nt35532_cs_of_match,
	},
};
module_mipi_dsi_driver(nt35532_cs_driver);

MODULE_AUTHOR("linux-mdss-dsi-panel-driver-generator <fix@me>"); // FIXME
MODULE_DESCRIPTION("DRM driver for nt35532 1080p cs video mode dsi panel");
MODULE_LICENSE("GPL");
