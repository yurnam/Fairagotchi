...
static const struct regulator_bulk_data nt35596_tianma_supplies[] = {
	{ .supply = "vsn" },
	{ .supply = "vsp" },
};
...
static int nt35596_tianma_on(struct nt35596_tianma *ctx)
{
	...
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0xd4, 0x0e);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0xff, 0x01);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0xfb, 0x01);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0x72, 0x21);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0x6d, 0x33);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0xff, 0x05);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0xfb, 0x01);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0xe7, 0x80);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0xff, 0x00);
...
static int nt35596_tianma_prepare(struct drm_panel *panel)
{
	...
	ret = regulator_bulk_enable(ARRAY_SIZE(nt35596_tianma_supplies), ctx->supplies);
	if (ret < 0) {
	...
	if (ret < 0) {
		...
		gpiod_set_value_cansleep(ctx->reset_gpio, 1);
		regulator_bulk_disable(ARRAY_SIZE(nt35596_tianma_supplies), ctx->supplies);
		return ret;
	}
...
static int nt35596_tianma_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx = devm_drm_panel_alloc(dev, struct nt35596_tianma, panel,
				   &nt35596_tianma_panel_funcs,
				   DRM_MODE_CONNECTOR_DSI);
	if (IS_ERR(ctx))
		return PTR_ERR(ctx);

	ret = devm_regulator_bulk_get_const(dev,
					    ARRAY_SIZE(nt35596_tianma_supplies),
					    nt35596_tianma_supplies,
					    &ctx->supplies);
	if (ret < 0)
		return ret;
