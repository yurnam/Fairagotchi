...
static const struct regulator_bulk_data ft8613_csot_5p7_supplies[] = {
	{ .supply = "vsn" },
	{ .supply = "vsp" },
};
...
static int ft8613_csot_5p7_on(struct ft8613_csot_5p7 *ctx)
{
	...
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0xf3,
				     0x40, 0x89, 0xc0, 0x40, 0x89, 0xc0, 0x40,
				     0x81, 0xc0, 0x40, 0x81, 0xc0);
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0x00, 0x00);
...
static int ft8613_csot_5p7_prepare(struct drm_panel *panel)
{
	...
	ret = regulator_bulk_enable(ARRAY_SIZE(ft8613_csot_5p7_supplies), ctx->supplies);
	if (ret < 0) {
	...
	if (ret < 0) {
		...
		gpiod_set_value_cansleep(ctx->reset_gpio, 1);
		regulator_bulk_disable(ARRAY_SIZE(ft8613_csot_5p7_supplies), ctx->supplies);
		return ret;
	}
...
static int ft8613_csot_5p7_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx = devm_drm_panel_alloc(dev, struct ft8613_csot_5p7, panel,
				   &ft8613_csot_5p7_panel_funcs,
				   DRM_MODE_CONNECTOR_DSI);
	if (IS_ERR(ctx))
		return PTR_ERR(ctx);

	ret = devm_regulator_bulk_get_const(dev,
					    ARRAY_SIZE(ft8613_csot_5p7_supplies),
					    ft8613_csot_5p7_supplies,
					    &ctx->supplies);
	if (ret < 0)
		return ret;
