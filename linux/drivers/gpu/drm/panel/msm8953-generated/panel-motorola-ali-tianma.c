...
static const struct regulator_bulk_data tianma_565_v0_supplies[] = {
	{ .supply = "vsn" },
	{ .supply = "vsp" },
};
...
static int tianma_565_v0_on(struct tianma_565_v0 *ctx)
{
	...
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0xb3, 0x04, 0x38, 0x08, 0x70);
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0x00, 0x00);
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0xff, 0xff, 0xff, 0xff);
	mipi_dsi_dcs_set_tear_on_multi(&dsi_ctx, MIPI_DSI_DCS_TEAR_MODE_VBLANK);
	mipi_dsi_dcs_set_display_brightness_multi(&dsi_ctx, 0x08cc);
...
static int tianma_565_v0_prepare(struct drm_panel *panel)
{
	...
	ret = regulator_bulk_enable(ARRAY_SIZE(tianma_565_v0_supplies), ctx->supplies);
	if (ret < 0) {
	...
	if (ret < 0) {
		...
		gpiod_set_value_cansleep(ctx->reset_gpio, 1);
		regulator_bulk_disable(ARRAY_SIZE(tianma_565_v0_supplies), ctx->supplies);
		return ret;
	}
...
static int tianma_565_v0_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx = devm_drm_panel_alloc(dev, struct tianma_565_v0, panel,
				   &tianma_565_v0_panel_funcs,
				   DRM_MODE_CONNECTOR_DSI);
	if (IS_ERR(ctx))
		return PTR_ERR(ctx);

	ret = devm_regulator_bulk_get_const(dev,
					    ARRAY_SIZE(tianma_565_v0_supplies),
					    tianma_565_v0_supplies,
					    &ctx->supplies);
	if (ret < 0)
		return ret;
